...
describe("buildDockerArgs", () => {
  ...
  it("exposes pnpm-managed Node bin on PATH and bare pnpm symlink so electron-builder collector can spawn npm/pnpm", () => {
    // Regression guard for "Node module collector process exited with code 127":
    // builder:base ships no npm/pnpm/corepack on PATH (only node via PNPM_HOME),
    // while electron-builder spawns the detected package manager directly.
    const args = buildDockerArgs(makeConfig(), { uid: 1000, gid: 1000 });
    const last = args[args.length - 1];
    expect(last).toContain("/nodejs/");
    expect(last).toContain("ln -sf /tmp/pnpm");
    expect(last).toContain("command -v npm >/dev/null");
    expect(last).toContain("command -v pnpm >/dev/null");
  });
